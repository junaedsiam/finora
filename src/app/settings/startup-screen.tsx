...
import {
  useSettingsStore,
  STARTUP_SCREEN_OPTIONS,
  type StartupScreen,
} from "@/stores/settings.store";
import { useColors } from "@/constants/colors";

export default function StartupScreenSetting() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const colors = useColors();
  const { startupScreen, setStartupScreen } = useSettingsStore();

  function handleSelect(id: StartupScreen) {
    setStartupScreen(id);
    router.back();
  }
    ...
    <View className="flex-1 bg-background" style={{ paddingTop: insets.top }}>
      ...
      <View className="px-4">
        {STARTUP_SCREEN_OPTIONS.map((option) => {
          const isSelected = option.id === startupScreen;
          return (
            <Pressable
              key={option.id}
